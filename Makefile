# Market Data Adapter Go - Makefile
...
teardown-test-db: ## Stop and remove test databases
	...
	@docker rm -f market-data-test-postgres market-data-test-redis || true
...
test-symbol: ## Run symbol tests only
	@echo "Running symbol tests..."
	go test -v ./tests -run TestSymbolBehaviorSuite -timeout=5m

test-price-feed: ## Run price feed tests only
	@echo "Running price feed tests..."
	go test -v ./tests -run TestPriceFeedBehaviorSuite -timeout=5m

test-candle: ## Run candle tests only
	@echo "Running candle tests..."
	go test -v ./tests -run TestCandleBehaviorSuite -timeout=5m

test-snapshot: ## Run market snapshot tests only
	@echo "Running market snapshot tests..."
...
check-env: ## Check test environment prerequisites
	...
	@echo "PostgreSQL connection: $(shell echo "SELECT 1" | psql "${TEST_POSTGRES_URL:-postgres://postgres:postgres@localhost:5432/market_data_test?sslmode=disable}" 2>/dev/null && echo "✓ OK" || echo "✗ Failed")"
...
export POSTGRES_URL ?= postgres://market_data_adapter:market-data-adapter-db-pass@localhost:5432/trading_ecosystem?sslmode=disable
export REDIS_URL ?= redis://market-data-adapter:market-data-pass@localhost:6379/0
