...
func (runner *BehaviorTestRunner) RunAllBehaviorTests(t *testing.T) {
	...
	testSuites := []struct {
		...
		{"SymbolBehavior", new(SymbolBehaviorTestSuite), false},
		{"PriceFeedBehavior", new(PriceFeedBehaviorTestSuite), false},
		{"CandleBehavior", new(CandleBehaviorTestSuite), false},
		{"MarketSnapshotBehavior", new(MarketSnapshotBehaviorTestSuite), false},
