...
func GetTestConfig() map[string]interface{} {
	return map[string]interface{}{
		"postgres_url": GetEnvOrDefault("TEST_POSTGRES_URL", "postgres://postgres:postgres@localhost:5432/market_data_test?sslmode=disable"),
...
// NextSymbol generates a unique trading symbol
func (g *TestDataGenerator) NextSymbol() string {
	g.counter++
	return fmt.Sprintf("TEST%d-USD", g.counter)
}

// NextSource generates a unique data source name
func (g *TestDataGenerator) NextSource() string {
	g.counter++
	return fmt.Sprintf("test-source-%d", g.counter)
}
